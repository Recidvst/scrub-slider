...
function Scrub(scrubArg) {
    ...
    function ScrubInitiate() {
        ...
        var scrubSlider = isObject(scrubArg) ? document.querySelectorAll(scrubArg.target)[0] : document.querySelectorAll(scrubArg)[0];
                ...
                if (scrubSlider != undefined) {
                    var createScrubImages = function createScrubImages(el, type, index) {
                        if (type == 'DIV') {
                            ...
                            // clone div
                            var _scrubImage = document.createElement('div');
                            var _imgSrc = el.getAttribute('src');
                            // if src specified manually
                            if (scrubArg.src != null) {
                                _imgSrc = index == 0 ? scrubArg.src[0] : scrubArg.src[1];
                            }
                            el.parentNode.removeChild(el);
                            // inject scrub container
                            var _scrubCont = document.createElement('div');
                            _scrubCont.className = index == 0 ? "scrub-content scrub-left" : "scrub-content scrub-right";
                            _scrubCont.style.width = parseInt(sliderWidth / 2) + "px";
                            // if alt specified
                            if (scrubArg.alt != null) {
                                _scrubImage.setAttribute('alt', index == 0 ? scrubArg.alt[0] : scrubArg.alt[1]);
                            }
                            scrubSlider.insertBefore(_scrubCont, scrubSlider.firstChild);
                            // re-attach div
                            _scrubImage.style.width = sliderWidth + "px";
                            _scrubImage.style.backgroundImage = 'url(' + _imgSrc + ')';
                            _scrubCont.append(_scrubImage);

                            // handle resize
                            var _reziseFn = debounce(function (scrubImage) {
                                var newSliderWidth = scrubSlider.offsetWidth;
                                _scrubCont.style.width = parseInt(newSliderWidth / 2) + "px";
                                scrubImage.style.width = newSliderWidth + "px";
                            }, 500);
                            window.addEventListener('resize', function (e) {
                                _reziseFn(_scrubImage);
                            });
                        }
                    };
